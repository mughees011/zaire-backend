    ...
    def __init__(self, groq_client, binance_api_key=None, binance_secret=None):
      ...
      if api_key and secret:
        try:
          ...
          self._write_broker_status(error_msg="No Binance keys provided")
      ...
      if alpaca_key and alpaca_secret:
          ...
          self._write_broker_status(error_msg="No Alpaca keys provided")
          print("[TRADER] No Alpaca keys provided — stocks analysis-only mode")

    def _write_broker_status(self, error_msg=None):
        import time, json, os
        if error_msg:
            current = getattr(self, "broker_error", "")
            self.broker_error = f"{current} | {error_msg}".strip(" |")
        ...
        status = {
            ...
            "error_msg": getattr(self, "broker_error", None)
        }
        try:
...
"""
    ...
    def _apex_daemon_loop(self):
        ...
        print("[TRADER APEX] Daemon thread started. ZAIRE is now watching 24/7.", flush=True)
        last_heartbeat_time = time.time()
        HEARTBEAT_INTERVAL  = 300    # seconds (5 minutes)
        CYCLE_SLEEP         = 15     # seconds between decision cycles
        control_path = os.path.join(os.path.dirname(__file__), '..', 'memory', 'trader_control.json')

        while True:
            # Sync active state from trader_control.json
            try:
                ...
                print(f"[TRADER APEX][WARNING] Failed to read control file: {e}")
                self.apex_active = False
            ...
            if now - last_heartbeat_time >= HEARTBEAT_INTERVAL:
                ...
                active_str = "ACTIVE" if self.apex_active else "PAUSED"
                print(f"[TRADER APEX][HEARTBEAT] Daemon alive ({active_str}). "
...
if __name__ == '__main__':
    print('[TRADER] Starting standalone daemon mode...')
    # Pass None because llm_utils handles AI calls natively via the vault
