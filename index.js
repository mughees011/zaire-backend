...
app.post('/engineer/scaffold', async (req, res) => {
  try {
    ...
    // AI Injection: Delegate generation to Python Design Intelligence Daemon
    try {
      console.log(`[ENGINEER SCAFFOLD] Spawning python engineer.py for: ${plan.appName}`);
      const payload = JSON.stringify({ plan, intake });
      
      const aiScaffold = await new Promise((resolve, reject) => {
        const { spawn } = require('child_process');
        const child = spawn('python', ['engineer.py', payload]);
        let stdoutData = '';
        let stderrData = '';
        ...
        child.on('close', (code) => {
          ...
          try {
            // Find the first occurrence of '{' to safely parse JSON in case of warnings
            const jsonStr = stdoutData.substring(stdoutData.indexOf('{'));
            const parsed = JSON.parse(jsonStr);
            if (parsed.error) reject(new Error(parsed.error));
            else resolve(parsed);
          } catch (e) {
            reject(new Error(`JSON Parse Error: ${e.message}. Output: ${stdoutData}`));
          }
        });
      });
      ...
      if (aiScaffold && aiScaffold.files) {
        for (const [key, val] of Object.entries(aiScaffold.files)) {
           ...
           scaffold.files[key].explanation = {
             what: `AI Generated ${key}`,
             why: 'Generated by ZAIRE Design Intelligence Core v4.0',
             edit: 'You can modify this file directly in the editor.',
             protect: 'Ensure design system tokens and anti-patterns are respected.'
           };
        }
      }
    } catch (aiErr) {
