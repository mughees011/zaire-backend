  ...
  try {
    ...
    try {
      ...
      const aiScaffold = await new Promise((resolve, reject) => {
        ...
        // 3-minute timeout — multi-step pipeline takes time
        const timeout = setTimeout(() => {
          child.kill();
          reject(new Error('Python orchestrator timed out after 180s'));
        }, 180000);
        ...
        child.on('close', (code) => {
          ...
          try {
            // Find the first '{' — skip any stderr warnings mixed into stdout
